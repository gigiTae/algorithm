...
void RedBlackTree::RightRotation(Info* x)
{
	Info* g = grandparent(x);
	Info* p = x->Parent;
...
void RedBlackTree::LeftRotation(Info* x)
{
	Info* g = grandparent(x);
	Info* p = x->Parent;
...
void RedBlackTree::InsertFix(Info* x)
{
	// Root 노드 
	if (x->Parent == nullptr)
	{
...
RedBlackTree::iterator& RedBlackTree::erase(iterator& iter) // 반환값은 중위후속자이다.
{
	...
	++Successoriter;
	Info* Successor = Successoriter.node;
...
void RedBlackTree::EraseFix(Info* x)
{
...
bool RedBlackTree::IsLeftChild(Info* x)
{
	Info* p = x->Parent;
	if (p == nullptr)
		return false;
...
bool RedBlackTree::IsRightChild(Info* x)
{
	Info* p = x->Parent;
	if (p == nullptr)
		return false;
...
Info* RedBlackTree::grandparent(const Info* n)
{
...
Info* RedBlackTree::uncle(const Info* n)
{
	Info* node = grandparent(n);
	if ( node == nullptr)
		return nullptr;
